...
10b. **Threads**: maintain the persistent story-arc registry (cross-day continuity the app renders as timelines). Fetch current threads from Supabase (`GET <SUPABASE_URL>/rest/v1/threads?select=slug,data` with the `apikey` header), link today's stories, write the complete result to `data/threads.json` (schema below). Linking is strictly evidence-gated — **a thread is a shared concrete anchor, never a vibe**:
    ...
    - **Do NOT scan from memory — run the surfacer.** `python3 scripts/scan_arcs.py --days 7` deterministically lists story PAIRS that share a concrete anchor: a near-identical headline (a re-report), the same street address, or the same RARE roster actor (a person/company in only ~2 stories). Entities come from the players roster (never geographies), and a shared $ figure or a common firm (Brookfield) never anchors alone. It reads Supabase only, so it runs in an egress-blocked sandbox. **Mandatory every run** — same as `scan_metrics.py` for metrics. It prints two lists: **HIGH-CONFIDENCE** (register the tale unless it's clearly wrong — patch the earlier days' `thread` fields and re-push) and **REVIEW** (same actor, possibly UNRELATED deals — apply the 5-anchor gate yourself and reject the same-player-unrelated ones, e.g. two different Hines deals). It's a FLOOR, not a ceiling: also read the day yourself for anchors it can't pattern-match (a shared site named differently, an event→resolution pair). A backfill on 2026-07-22 found 9 tales it now catches (RealPage/Willow Bridge suit, Peter Fine 105 Willis, Boxabl IPO, …).
