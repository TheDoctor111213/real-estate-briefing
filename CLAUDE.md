...
9. **Players roster**: maintain the persistent people/companies dossier set behind the app's Players tab. Fetch the current roster from Supabase (`GET <SUPABASE_URL>/rest/v1/players?select=slug,data` with the `apikey` header), merge today's stories into it, write the complete result to `data/players.json` (schema below). Curation rules — these keep the roster valuable instead of unwieldy:
   ...
   - **Profile image** (`image` field): source a stable square image URL when creating a profile. **The reliable one-shot is `python3 scripts/backfill_player_images.py` (or `--slug <slug> …`)** — it sources a logo/headshot, re-hosts it in the `player-images` bucket, and sets `image`. It uses `unavatar.io/<domain>?fallback=false` (aggregates favicon/logo/clearbit-cache and returns a REAL logo where a bare Google favicon comes back blank — the historical failure mode) with DuckDuckGo/Google fallbacks, and Wikipedia for people. Add the entity's domain to that script's `COMPANY_DOMAINS` (or a person to `PEOPLE_WIKI`) when the name doesn't map cleanly. Manual order of preference if sourcing by hand:
